...
func (c *Client) GetBlockByNumber(ctx context.Context, blockNum string, fullTx bool) (*Block, *CallResult) {
	block, _, result := c.GetBlockByNumberWithRaw(ctx, blockNum, fullTx)
	return block, result
}
...
func (c *Client) GetBlockByNumberWithRaw(ctx context.Context, blockNum string, fullTx bool) (*Block, json.RawMessage, *CallResult) {
	...
	// Check if block exists (null result means block not found)
	if string(rawResponse) == "null" {
		result.Success = false
		result.Error = fmt.Errorf("block not found")
		result.ErrorType = ErrorTypeRPCError
		return nil, nil, result
	}
	...
	if err := json.Unmarshal(rawResponse, &blockData); err != nil {
		result.Success = false
...
// parseHexBigInt converts a hex string (with or without 0x prefix) to *big.Int
