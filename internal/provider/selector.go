...
type ProviderHealth struct {
	...
	BlockHeight  uint64
	BlockDelta   int     // Blocks behind max
...
func QuickHealthCheck(ctx context.Context, cfg *config.Config, samples int) (RankedProviders, error) {
	...
	_ = g.Wait() // Continue even if some providers fail
	...
	for name, pd := range providerData {
		...
		health.Score = calculateScore(health, maxHeight)
...
func (rp RankedProviders) Best() (ProviderHealth, error) {
	...
	// All excluded - return best anyway with warning
	if len(rp) > 0 {
...
func calculateScore(h ProviderHealth, maxHeight uint64) float64 {
	// Success rate: 50% weight (0-100 -> 0-1)
	successScore := h.SuccessRate / 100.0

	// Latency: 30% weight (lower is better, cap at 1000ms)
	...
	// Freshness: 20% weight (0 blocks behind = 1.0)
