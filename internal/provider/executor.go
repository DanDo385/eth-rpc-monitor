// Package provider provides utilities for concurrent provider operations.
// It abstracts the common pattern of executing operations across all providers
// with proper concurrency handling, result ordering, and client reuse.
package provider
...
// Example:
//
//	results := provider.ExecuteAll(ctx, cfg, pool, func(ctx context.Context, client *rpc.Client, p config.Provider) WatchResult {
