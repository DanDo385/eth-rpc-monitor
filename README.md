...
A small **Go** CLI for Ethereum **JSON-RPC over HTTP**: measure **tail latency** (P50 / P95 / P99 / Max), **reliability**, and **cross-provider agreement** on the same block—using one YAML config and no heavy SDK so behavior stays visible.

**What you get** — a single `ethrpc` binary with four subcommands:

- **`ethrpc block`** — One block from the best auto-selected provider (highest head, lowest latency among ties) or a pinned provider.
- **`ethrpc test`** — Many samples per provider, colored table, height-drift warning; optional JSON report.
- **`ethrpc snapshot`** — Same block tag from everyone; height and hash mismatch detection.
- **`ethrpc monitor`** — Live terminal dashboard with intentional “cold” client per tick for realistic poll cost.
...
The market also doesn't sleep. State can be queried at any time, which is exactly what `./bin/ethrpc snapshot latest` and `./bin/ethrpc monitor` are for: an honest answer to "what does the chain look like *right now*?" with no business-hours dependency.
...
4. **Secrets** — either `export` variables before running or add a **`.env`** in the project root. The CLI calls `config.LoadEnv()` on startup. See **`.env.example`** for common variable names.
...
```bash
make build        # produces bin/ethrpc
...
**Tech stack:** Go 1.24+, `github.com/spf13/cobra` (CLI), `golang.org/x/sync/errgroup`, `gopkg.in/yaml.v3`, `github.com/fatih/color` for terminal output.

---

## 7. Commands

All subcommands run via the single `ethrpc` binary. **`--config <path>`** (also `-c`) is a persistent flag on the root, defaults to `config/providers.yaml`, and is applied before any subcommand runs. cobra conventions: long flags use a double dash (`--samples`), shorthands use a single dash (`-s`); shorthand aliases are noted per subcommand.
...
```bash
./bin/ethrpc block                    # same as ./bin/ethrpc block latest
./bin/ethrpc block latest
./bin/ethrpc block pending
./bin/ethrpc block earliest
./bin/ethrpc block 19000000           # decimal height
./bin/ethrpc block 0x121eac0          # hex height
./bin/ethrpc block latest --provider alchemy
./bin/ethrpc block latest --json      # reports/block-YYYYMMDD-HHMMSS.json
```

**Flags:** `--config`/`-c`, `--provider <name>`, `--json`/`-j`
...
```bash
./bin/ethrpc test                     # sample count from config (health_samples)
./bin/ethrpc test --samples 10
./bin/ethrpc test --json              # reports/health-YYYYMMDD-HHMMSS.json
```

**Flags:** `--config`/`-c`, `--samples <n>`/`-s`, `--json`/`-j`
...
./bin/ethrpc snapshot latest
./bin/ethrpc snapshot 0x121eac0       # hex block tag
```

**Note:** Prefer **`latest`** or **hex** here; decimal tags are not normalized the way they are in **`block`**. Use **`ethrpc block`** for flexible decimal/hex on a single provider.

**Flags:** `--config`/`-c` only (no `--json` in this tool).
...
```bash
./bin/ethrpc monitor                  # interval from config (watch_interval)
./bin/ethrpc monitor --interval 10s   # override refresh
```

**Flags:** `--config`/`-c`, `--interval <duration>`/`-i` — use **`0`** to use the YAML `watch_interval` default.

---

## 8. JSON reports (`block` and `test` only)

With **`--json`** (or **`-j`**), reports are written under **`reports/`** (created if needed), timestamped, and pretty-printed. Diagnostics stay on **stderr** so scripts can rely on stdout/file behavior.

```bash
./bin/ethrpc block latest --json
./bin/ethrpc test --json
```
...
A self-hosted node (Geth, Nethermind, Reth, …) typically removes one network hop and the shared-rate-limit risk that comes with public endpoints; the realistic latency floor lives there, often single-digit ms. The example YAML at [`config/providers.yaml.example`](config/providers.yaml.example) ships a commented **`local-geth`** entry pointed at `http://localhost:8545` precisely so you can drop in your own node and compare it side-by-side against vendor URLs in the same `./bin/ethrpc test` table.
...
| First run | `./bin/ethrpc test --samples 3` |
...
| Symptom | What to try |
|---------|--------------|
| `failed to read config: no such file` | Copy the example YAML; or `./bin/ethrpc block --config /absolute/path/to/providers.yaml` |
...
| Path | Role |
|------|------|
| `cmd/ethrpc` | cobra CLI entrypoint — root + `block` / `test` / `snapshot` / `monitor` subcommands (thin flag wiring) |
| `internal/cli` | Command workflows (`RunBlock` / `RunTest` / `RunSnapshot` / `RunMonitor`) + report types; importable, tested |
